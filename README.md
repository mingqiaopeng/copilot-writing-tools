...
在此领域，深入使用后暴露出的不是"写得不够快"，而是更深层的质量问题。概括起来，**六大痛点**尤为突出：
...
软件开发者有 IDE——编码、调试、测试、部署一站式完成。写作者却没有类似的一体化平台：构思在浏览器里，起草在 Word 里，审核靠人工来回批注。本项目选择 VS Code + GitHub Copilot 这个组合，正是要填补这个空缺——为严肃写作者提供一个"写作 IDE"。
